...
** Helm
   - Configure [[https://github.com/emacs-helm/helm][helm]], an incremental completion and selection narrowing
     ...
     #+BEGIN_SRC emacs-lisp
       ...
       (customize-set-variable 'helm-grep-ag-command "rg --smart-case --search-zip --no-heading --line-number %s -- %s %s")
     #+END_SRC
...
** Theme
   ...
   - Use better faces for theme.

     #+begin_src emacs-lisp
       (custom-theme-set-faces 'zenburn `(company-preview ((t (:inherit company-tooltip-selection)))))
     #+end_src
