   ...
   - Use ~org-publish~ to manage my [[http://xiongtx.github.io][GitHub blog]]. We need to set the
     ...
     #+BEGIN_SRC emacs-lisp
       (customize-set-variable
        ...
        '(("blog posts"
           :base-directory "~/Dropbox/github/xiongtx.github.io/_org/"
           :base-extension "org"
           :body-only t
           :html-extension "html"
           :publishing-directory "~/Dropbox/github/xiongtx.github.io/"
   ...
   - Initializations for =eshell=.
     ...
     #+BEGIN_SRC emacs-lisp
       (defun *-eshell-kill-input ()
         "Kill current input at an `eshell' prompt."
         (interactive)
         (eshell-bol)
         (kill-line))
   ...
   - Function to easily switch Eshell directory to other window's buffer's
     ...
     #+BEGIN_SRC emacs-lisp
       (defun *-eshell-other-window-buffer-file-directory ()
         "Return other window's buffer's file directory, if it exists. NIL
       otherwise.

       If the other window's buffer is a Dired buffer, switch to its
       directory."
...
** JavaScript
   ...
   - Use =tern= with =company=.

     #+BEGIN_SRC emacs-lisp
       ;; Use tern with js2-mode
       (add-hook 'js2-mode-hook (lambda () (tern-mode t)))

       ;; Use company-mode with tern
       (with-eval-after-load 'company
...
** Magit
   - Bind the =magit-status= command, which serves as Magit's entry point.
...
** Nand2Tetris
   ...
   - Configs for [[https://github.com/CestDiego/nand2tetris.el/][nand2tetris.el]] for the [[https://www.coursera.org/learn/build-a-computer/][Build a Modern Computer from First Principles]] course.

     #+BEGIN_SRC emacs-lisp
       (customize-set-value 'nand2tetris-core-base-dir "~/Dropbox/coursera/build-a-computer/nand2tetris")
