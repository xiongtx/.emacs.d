...
** Font
   - Change the default font. Use a hook to ensure consistent sizing for both
     direct Emacs and emacsclient frames.

     #+BEGIN_SRC emacs-lisp
       (defun *-set-font (&optional frame)
         (let ((height (if (string= system-type "darwin") 120 110)))
           (set-face-attribute 'default frame :family "DejaVu Sans Mono" :height height)))
...
** Helm
   - Configure [[https://github.com/emacs-helm/helm][helm]], an incremental completion and selection narrowing
     ...
     #+BEGIN_SRC emacs-lisp
       ...
       ;; Don't ask for confirmation when creating helm-grep buffer
       (customize-set-variable 'helm-grep-save-buffer-name-no-confirm t)
     #+END_SRC
...
** Python
   ...
   - Use [[https://github.com/joaotavora/eglot][eglot]] (built-in LSP client) with [[https://github.com/microsoft/pyright][pyright]] for Python development.
     Install pyright globally: =uv tool install pyright=

     #+BEGIN_SRC emacs-lisp
       (add-hook 'python-mode-hook #'eglot-ensure)
       ;; Keep eldoc active but don't display in echo area; use C-h C-j for docs
   ...
   - Helper function to set the Python interpreter from a uv virtual environment.

     #+BEGIN_SRC emacs-lisp
       (defun *-python-set-venv (venv-path)
         "Set Python interpreter to VENV-PATH's python and restart eglot."
         (interactive "DSelect venv directory: ")
   ...
   - Auto-detect uv/standard .venv in project root for the REPL.

     #+BEGIN_SRC emacs-lisp
       (defun *-python-auto-venv ()
         "Automatically activate .venv if present in project root."
       ...
       (add-hook 'python-mode-hook #'*-python-auto-venv)
     #+END_SRC

   - Format with [[https://github.com/psf/black][black]].
     ...
     #+BEGIN_SRC emacs-lisp
       (defun *-python-format-buffer ()
         ...
         (if (executable-find "black")
           ...
           (message "black not found in PATH")))

       (with-eval-after-load 'python
         (define-key python-mode-map (kbd "C-c C-f") #'*-python-format-buffer))
     #+END_SRC

   - Switch to Python shell, starting one if needed (like elpy's =C-c C-z=).
     In the shell, =C-c C-z= switches back to the source buffer.

     #+BEGIN_SRC emacs-lisp
       (defun *-python-shell-switch-to-shell ()
         "Switch to inferior Python process buffer, starting one if needed."
         (interactive)
       ...
       (with-eval-after-load 'python
         ...
         ;; Elpy-style send bindings
         (define-key python-mode-map (kbd "C-x C-e") #'python-shell-send-statement)
         (define-key python-mode-map (kbd "C-M-x") #'python-shell-send-defun)
         (define-key python-mode-map (kbd "C-c C-r") #'python-shell-send-region))
     #+END_SRC

   - Display Python documentation in a popup using eldoc.

     #+BEGIN_SRC emacs-lisp
       (defun *-python-eldoc-doc (_symbol)
         "Get documentation from eldoc buffer."
         (if (not (eglot-managed-p))
             ""
           ;; Force eldoc to refresh
           (eldoc-print-current-symbol-info)
           (sit-for 0.1)
           ;; eldoc-doc-buffer creates the buffer if needed
           (if-let ((buf (eldoc-doc-buffer)))
               (with-current-buffer buf
                 (buffer-string))
             "")))

       (defun *-python-doc-popup ()
         "Display Python documentation in a popup."
         (interactive)
         (funcall (*-make-doc-command #'*-python-eldoc-doc
                                      (lambda () (thing-at-point 'symbol t)))))

       (with-eval-after-load 'python
         (define-key python-mode-map (kbd "C-h C-j") #'*-python-doc-popup))
     #+END_SRC
